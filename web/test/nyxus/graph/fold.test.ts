...
describe('Agent-local Fold projection', () => {
  ...
  it('keeps the global graph canonical and renders Fold with the shared node skin plus wheel', async () => {
    ...
    // 左轮原样回归（2026-09-02 二轮返工）：143af38~1 行为排版原样，仅直角 + 卡内动效适配。
    expect(railSource).not.toContain('border-radius')
    ...
    // 340ms cubic-bezier 轮转过渡与 left/top 定位属旧版原样豁免；scale( 仅允许
    // 出现在卡内 unread pop 关键帧（3d 轨道仍然禁止）。
    ...
    expect(railSource).not.toContain('scale3d(')
