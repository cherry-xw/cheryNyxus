 ...
 * 不把子消息复制进父实体（不变式 2）。
 *
 * 群聊合并复用 [historyMerge](../agents/data/historyMerge.ts) 的纯逻辑（collectDescendantChatIds
 * + dedupHistoryByMsgId + mergeChildReplyHistory），通过 `toHistoryItem` 把规范 ChatMessage
