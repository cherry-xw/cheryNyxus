...
export function createUiState() {
  ...
  // 当前「活跃」的 Cherry Nexus 会话 id（多会话模型下桌宠绑定此会话）。null 时 selector 回退到最近一条。
  ...
  // Nexus 专属历史面板（独立于用户会话列表 SessionList）。
  const nyxusHistoryOpen = ref(false)
   ...
   *  优先级：settings > agentDialog > historyDrawer > nyxusHistory > sessionList。 */
  const topOverlay: ComputedRef<OverlayKind | null> = computed(() => {
    ...
    if (nyxusHistoryOpen.value) return 'nyxusHistory'
