...
export function mergeChildReplyHistory(items: readonly HistoryItem[]): HistoryItem[] {
  ...
  for (const returned of returns) {
    const candidateIndices = [...finalReplyIndices].filter((candidateIndex) => {
      ...
      return sameReplyContent(returned.item.content, candidate.content)
    ...
    if (nearest.length > 1 && replyDistance(nearest[0]!.item, returned.item) === replyDistance(nearest[1]!.item, returned.item)) {
      continue;
    }
...
function replyDistance(childReply: HistoryItem, returned: HistoryItem): number {
  if (childReply.createdAt === undefined || returned.createdAt === undefined) return Number.MAX_SAFE_INTEGER;
