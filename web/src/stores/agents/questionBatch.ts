...
function toBatchState(
  ...
  const previousItems = new Map(previous?.questions.map((question) => [question.questionId, question]));
  return {
    ...
    questions: [...payload.questions]
      ...
      .map((question): QuestionItemState => {
        ...
        return {
          ...
          ...(old?.draftAnswer ? { draftAnswer: { ...old.draftAnswer, selectedLabels: [...old.draftAnswer.selectedLabels] } } : {}),
        };
      }),
  };
}
...
export function ensureActiveQuestion(stream: StreamState): void {
  ...
  stream.activeQuestionId = stream.questionBatches
