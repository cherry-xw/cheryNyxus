...
const remoteMode = computed(
  () => props.page !== undefined && props.pageSize !== undefined && props.total !== undefined,
)
...
const visibleItems = computed(() => props.items.slice(windowStart.value, windowEnd.value))
...
function scrollTo(item: IndexItem, i: number): void {
  ...
  const newStart = Math.max(
    0,
    Math.min(props.items.length - visibleCount.value, globalIndex - half),
  )
...
<template>
  <div v-if="remoteMode" class="shell-index-row paginated remote-pager">
    <button
      ...
      aria-label="上一页"
      @click="emit('page-change', remotePage - 1)"
    >
      <ArrowLeft class="ico" />
    </button>
    <button
      v-for="p in remotePages"
      :key="p"
      type="button"
    ...
    <span class="shell-index-page-info"
      >{{ (remotePage - 1) * remotePageSize + 1 }}-{{
        Math.min(remotePage * remotePageSize, remoteTotal)
      }}
      / {{ remoteTotal }}</span
    >
    <button
      type="button"
      class="shell-index-nav"
      :disabled="remotePage >= remotePageCount"
      aria-label="下一页"
      @click="emit('page-change', remotePage + 1)"
    >
  ...
  <div v-else-if="needPagination" ref="containerRef" class="shell-index-row paginated">
    ...
    <span class="shell-index-page-info"
      >{{ windowStart + 1 }}-{{ windowEnd }} / {{ items.length }}</span
    >
    <button
      type="button"
      class="shell-index-nav"
