  ...
  <div class="neon-number" :title="tip">
    ...
    <div class="neon-number-console">
      ...
      <button v-else type="button" class="number-readout" @click="startEdit">
        <b>{{ modelValue ?? placeholder }}</b
        ><small v-if="modelValue !== undefined && unit">{{ unit }}</small>
      </button>
...
.neon-number-console > button:first-child {
  border-right: 1px solid rgba(99, 102, 241, 0.1);
}
.neon-number-console > button:last-child {
  border-left: 1px solid rgba(99, 102, 241, 0.1);
}
.neon-number-console > button:hover {
  background: rgba(99, 102, 241, 0.09);
  text-shadow: 0 0 7px rgba(99, 102, 241, 0.5);
}
.number-readout {
  display: flex;
  align-items: center;
