<template>
  <div class="tag-select">
    <span class="lbl">
      {{ label }}
      <span v-if="tokenEstimate != null && tokenEstimate > tokenWarning"
            class="token-badge"
    ...
    <div class="tag-select-tags">
      <el-tag v-for="item in displayed" :key="item"
              size="small" type="info" closable
              @close="remove(item)">
        {{ item }}
      </el-tag>
      <span v-if="overflow > 0"
            class="tag-select-more"
            v-popover:more>
        +{{ overflow }} 更多
      </span>
      <el-popover ref="more" :width="200" trigger="hover">
        <div style="display:flex;flex-wrap:wrap;gap:4px;">
          <el-tag v-for="item in modelValue" :key="item"
                  size="small" type="info" closable
                  @close="remove(item)">
            {{ item }}
          </el-tag>
    ...
    <div class="tag-select-input">
      <el-select :model-value="[]"
                 filterable multiple size="small"
                 placeholder="添加..."
                 @change="onAdd">
