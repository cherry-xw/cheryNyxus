...
const PATTERNS: Array<{ kind: ImportErrorKind; re: RegExp }> = [
  {
    kind: 'proxy',
    re: /代理|proxy|connection timed out|failed to connect to|could not resolve host|etimedout|econnrefused/i,
  },
  {
    kind: 'auth',
    re: /鉴权|认证|authentication failed|permission denied|invalid username|forbidden|401|403|token/i,
  },
...
const META: Record<ImportErrorKind, { title: string; detail: string; hint?: string }> = {
  proxy: {
    title: '代理连接失败',
    detail: '无法通过当前网络拉取仓库。',
    hint: '在下方「网络代理」填入地址后重试。',
  },
  auth: {
    title: '鉴权失败',
    detail: '用户名 / Token 无效或无权限。',
    hint: '检查凭据或重新输入 Token。',
  },
  git: {
    title: '缺少 git',
    detail: '未检测到 git CLI，Git 导入不可用。',
    hint: '请先安装 git 命令行。',
  },
