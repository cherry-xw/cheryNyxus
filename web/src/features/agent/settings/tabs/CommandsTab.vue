...
import { formatThreshold, formatTokenCount, parseThreshold, parseTokenCount } from "@/features/agent/commands";
...
function filterThresholdInput(value: string, target: "warn" | "auto" | "min"): void {
  const ok =
    value === "" ||
    value === "." ||
    THRESHOLD_PREFIX_RE.test(value);
  if (!ok) {
...
const indexItems = computed<IndexItem[]>(() => [
  { label: "compact", anchor: "compact", description: "上下文过长时整理关键事实与进度，避免信息丢失" },
...
function applyWarn(): void {
  ...
  if (!parsed) {
    ...
    warnText.value = formatThreshold(props.draft.global.command?.warn ?? { unit: "percent", value: 0.6 });
    return;
  }
...
function applyAuto(): void {
  ...
  if (!parsed) {
    ...
    autoText.value = formatThreshold(props.draft.global.command?.auto ?? { unit: "percent", value: 0.8 });
    return;
  }
  ...
  <TabShell tab-key="commands" :index-items="indexItems">
    ...
    <article class="card compact-card" data-anchor="compact">
      ...
      <p v-if="!compactAvailable" class="unavail-hint">
        <code>.chery/command/compact.md</code> 不存在，compact 指令不可用。阈值配置禁用；恢复文件后刷新列表即可。
      </p>

      <div class="field-row compact-row">
        <label class="field">
          <LabelTip label="警告阈值" tip="上下文占用达到这个比例时，界面会提示你，但不会自动压缩。填 60% 或 96k" />
          <el-input
            v-model="warnText"
        ...
        <label class="field">
          <LabelTip label="自动压缩" tip="上下文占用达到这个比例时，会自动压缩上下文。填 80% 或 100k" />
          <el-input
            v-model="autoText"
        ...
        <label class="field">
          <LabelTip label="最小上下文容量" tip="brain 的总容量低于这个值时，compact 功能会自动禁用，防止压爆上下文。填 32k" />
          <el-input
            v-model="minText"
    ...
    <article v-if="!loadingList && commands.length === 0" class="card empty-card">
      ...
      <p class="empty-hint">
        <code>.chery/command/</code> 目录为空，compact 不可用。在文件系统新增 <code>compact.md</code> 后点「刷新列表」。
      </p>
    </article>
