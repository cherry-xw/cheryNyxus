...
export const HEADER_LAYERS: readonly HeaderLayer[] = [
  { id: 'loop', title: '循环调度' },
  { id: 'record', title: '内容记录', parent: 'loop' },
...
const n = (
  ...
  kinds: WorkflowStepKind[],
  detail: string,
  match?: HeaderTemplateNode['match'],
): HeaderTemplateNode => ({
  id,
  group,
  title,
  ...
  shape:
    match === 'unobserved'
      ? 'note'
      : ['decision', 'response', 'error', 'approval-needed'].includes(id)
        ? 'condition'
        : 'step',
})
const nodes = [
  n(
    ...
    '本轮入口',
    [],
    '每次 Loop 从这里进入完整中间件链；这是结构边界，没有独立步骤记录。',
    'unobserved',
  ),
  ...
  n(
    ...
    '收集适用的失败或拒绝，不等待所有来源，也不表示后续步骤已执行。',
    'rejection',
  ),
  n(
    'resume',
...
const edges = [
  ...
  e('queue', 'entry'),
  e('entry', 'input'),
  e('input', 'command', '有指令', 'condition'),
  e('command', 'request'),
  e('input', 'request', '无指令', 'condition'),
  ...
  e('error', 'result', '不可恢复 / 耗尽', 'retry'),
  e('tool-list', 'validation'),
  e('validation', 'authorization'),
  e('authorization', 'approval-needed'),
  e('approval-needed', 'approval', '需要', 'condition'),
  e('approval-needed', 'preflight', '无需', 'condition'),
  e('approval', 'preflight', '通过', 'condition'),
  e('preflight', 'execution'),
  e('execution', 'tool-result'),
  ...['validation', 'authorization', 'approval', 'preflight', 'execution'].map((id) =>
    e(id, 'rejection', id === 'approval' ? '拒绝' : '失败', 'condition'),
  ),
  e('rejection', 'tool-result', '记录结果'),
  e('tool-result', 'checkpoint'),
  e('checkpoint', 'decision'),
  e('decision', 'entry', '下一轮', 'loop'),
  e('decision', 'wait', '等待', 'loop'),
  e('decision', 'result', '完成', 'condition'),
  e('wait', 'wake', '实际唤醒', 'loop'),
  e('wake', 'entry', '进入本轮', 'loop'),
  e('resume', 'tool-list', '重建调用'),
  e('resume', 'tool-result', '工具失效', 'condition'),
  e('execution', 'dispatch', '派发', 'collaboration'),
  e('dispatch', 'child-run', '子执行', 'collaboration'),
  e('child-run', 'child-return', '回传', 'collaboration'),
  e('child-return', 'parent-receive', '接收', 'collaboration'),
  e('parent-receive', 'wake', '明确唤醒', 'collaboration'),
  e('parent-receive', 'input', '实际消费', 'collaboration'),
  e('request', 'compact-request', '压缩请求', 'compact'),
  e('compact-request', 'compact-summary', '生成', 'compact'),
  e('compact-summary', 'compact-applied', '采用', 'compact'),
...
export interface HeaderNodePort {
  id: string
  type: 'source' | 'target'
  side: HeaderPort
  offset: number
