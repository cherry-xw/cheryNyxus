...
export function moveHeaderPath(points: readonly HeaderPoint[], source: HeaderPoint, target: HeaderPoint): HeaderPoint[] {
  ...
  result[0] = source; result[result.length - 1] = target
  ...
  for (const point of result) {
    ...
    if (previous && previous.x !== point.x && previous.y !== point.y) orthogonal.push({ x: point.x, y: previous.y })
...
export function headerCrossingPath(points: readonly HeaderPoint[], previousPaths: readonly (readonly HeaderPoint[])[]): string {
  const commands: string[] = points.length ? [`M ${points[0]!.x} ${points[0]!.y}`] : []
  for (let i = 1; i < points.length; i++) {
    const a = points[i - 1]!, b = points[i]!, horizontal = a.y === b.y, axis = horizontal ? 'x' : 'y'
    const cuts = new Set<number>()
    for (const other of previousPaths) for (let j = 1; j < other.length; j++) {
      const c = other[j - 1]!, d = other[j]!
      if (horizontal === (c.y === d.y)) continue
      const x = horizontal ? c.x : a.x, y = horizontal ? a.y : c.y
      if (x > Math.min(a.x, b.x) && x < Math.max(a.x, b.x) && y > Math.min(c.y, d.y) && y < Math.max(c.y, d.y) && horizontal) cuts.add(x)
      if (!horizontal && y > Math.min(a.y, b.y) && y < Math.max(a.y, b.y) && x > Math.min(c.x, d.x) && x < Math.max(c.x, d.x)) cuts.add(y)
    }
    ...
    for (const cut of [...cuts].sort((x, y) => (x - y) * direction)) {
      ...
      commands.push(horizontal ? `L ${cut - direction * 4} ${a.y} M ${cut + direction * 4} ${a.y}` : `L ${a.x} ${cut - direction * 4} M ${a.x} ${cut + direction * 4}`)
    }
