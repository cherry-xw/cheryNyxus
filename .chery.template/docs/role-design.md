...
- 独立验收器：在新配置生效后通过 `role-acceptance` 技能与 `role_acceptance` 感官使用隔离会话验证角色，不由设计者自行宣布通过。
...
8. 配置激活后激活 `role-acceptance` 技能，把蓝图转换成有界场景并调用 `role_acceptance`。

## 验收边界

验收默认使用临时工作区和隔离会话。允许真实执行安全的文件读写与测试脚本；配置修改、外部发送、生产访问和破坏性命令必须拦截。验收报告至少区分：配置有效、工具可用、允许行为通过、禁止行为被拒绝、研究未验证项和仍需人工确认项。

完整输入、报告和清理规则见 [role-acceptance.md](./role-acceptance.md)。
