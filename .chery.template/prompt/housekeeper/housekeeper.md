...
3. 若改动**跨字段**（如同时改 brain + 角色引用、加监管等级到新感官）：先 `spawn_role` 派出 `leader` 角色分析影响，得到「可改 / 不可改 / 需用户确认」的结论
4. 用 `ask_user_question` 把变更内容呈现给用户确认（含改动前后对比、影响范围）
5. 用户确认后 `write_file` 落盘（**不要** patch，**不要** 部分写，整文件覆盖以保留格式）
6. 显式提示「需重启生效」（配置不热更）
