...
import {
  createPrivateServices,
  renderRatholeClientConfig,
  renderRatholeServerConfig,
...
describe('rathole configuration', () => {
  it('creates only private HTTP and WebSocket services', () => {
    ...
    expect(services[0]?.token).toBe(services[1]?.token)
  })

  it('maps accepted lease data to two loopback-only services', () => {
    const services = servicesFromAccepted({
      accepted: {
        tunnel: {
          httpService: 'http-lease',
          websocketService: 'ws-lease',
          token: 't'.repeat(32),
        },
      },
      addresses: {
        httpLocalAddr: '127.0.0.1:8183',
        websocketLocalAddr: '127.0.0.1:8182',
      },
      bindAddresses: {
        httpBindAddr: '127.0.0.1:48081',
        wsBindAddr: '127.0.0.1:48082',
    ...
    expect(services.map((service) => service.name)).toEqual(['http-lease', 'ws-lease'])
    expect(renderRatholeClientConfig({ serverAddr: 'relay.example:2333', services })).not.toContain('39980')
    expect(() => servicesFromAccepted({
      accepted: { tunnel: { httpService: 'http-lease', websocketService: 'ws-lease', token: 't'.repeat(32) } },
      addresses: { httpLocalAddr: '127.0.0.1:39980', websocketLocalAddr: '127.0.0.1:8182' },
      bindAddresses: { httpBindAddr: '127.0.0.1:48081', wsBindAddr: '127.0.0.1:48082' },
    })).toThrow(/manager port/)
  })
})
