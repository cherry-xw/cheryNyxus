...
<style>body{max-width:920px;margin:0 auto;padding:32px;background:#11151b;color:#e8ecf8;font:15px system-ui,sans-serif}main{display:grid;gap:16px}section{border:1px solid #34404e;padding:20px;background:#171d25}h1{margin-top:0}h2{font-size:18px;margin:0 0 12px}pre{white-space:pre-wrap;overflow-wrap:anywhere}button,input{font:inherit;padding:8px 10px;border:1px solid #59687b;background:#202a36;color:inherit}button{cursor:pointer}button:focus-visible{outline:2px solid #22d3ee;outline-offset:2px}form{display:flex;flex-direction:column;gap:10px;max-width:520px}.field{display:flex;flex-direction:column;gap:4px}label{color:#94a3b8;font-size:13px}.row{display:flex;gap:8px}.row input{flex:1}ul{list-style:none;padding:0;margin:0 0 12px}li{padding:4px 0}.ok{color:#4ade80}.warn{color:#fbbf24}.error{color:#f87171}.muted{color:#94a3b8}.lan-warning{background:rgba(251,191,36,.15);border:1px solid #fbbf24;color:#e8ecf8;padding:14px 16px;border-radius:6px;line-height:1.6}.lan-warning strong{color:#fbbf24;display:inline-flex;align-items:center;gap:6px;margin-bottom:4px}.lan-warning svg{flex:none;width:18px;height:18px;fill:#f87171}.primary{background:#22d3ee;border-color:#22d3ee;color:#04202b;font-weight:600}#message{margin:10px 0 0}@media(max-width:640px){body{padding:16px}section{padding:16px}.row{flex-wrap:wrap}.row input{flex:1 1 100%}}</style>
...
<form id="rotate"><div class="field"><label for="username">用户名（后端 Web 登录，必填）</label><input id="username" autocomplete="username" placeholder="如 admin"></div><div class="field"><label for="password">新密码（后端 Web 登录，必填；留空提交会自动生成随机密码）</label><div class="row"><input id="password" type="password" autocomplete="new-password" placeholder="留空自动生成"><button id="toggle-password" type="button">显示</button><button id="random-password" type="button">随机生成</button></div></div><button type="submit" class="primary">保存凭据并生效</button></form>
<p id="message" role="status" aria-live="polite"></p></section></main>
<script>
...
$('refresh').onclick=load;load();setInterval(load,10000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)load()});
</script>`)
}
