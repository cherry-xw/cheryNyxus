...
function evictMemory(
  ...
  writeMemoryIndex(index.filter((e) => e.filename !== name), workspace, scope);
}
...
export function addMemory(params: {
  ...
  const {
    name, description, content, type, originSessionId, replaceTarget, replaceReason, workspace,
    scope = "workspace",
  } = params;
...
/** 列出该层所有历史记忆 */
export function listHistories(workspace?: string, scope: MemoryScope = "workspace"): HistoryEntry[] {
