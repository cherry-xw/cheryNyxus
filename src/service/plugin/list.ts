...
export function listPluginSkills(pluginName: string, dir: string): PluginSkillInfo[] {
  ...
  for (const loc of discoverSkillRoots(dir)) {
    const skillData = loadSkillFromFolder(loc.skillDir, {
      plugin: pluginName,
...
export function buildPluginInfo(name: string): PluginInfo | undefined {
  ...
  const minContentTokens = skills.length > 0 ? Math.min(...skills.map((s) => s.contentTokens)) : 0
  const maxContentTokens = skills.length > 0 ? Math.max(...skills.map((s) => s.contentTokens)) : 0
  return {
    name,
