 ...
 */
export function createHttpServer({ webPort, staticDir, sessionToken, host = "127.0.0.1", auth }: CreateHttpServerOptions) {
...
async function handleRequest(
  ...
  if (auth?.enabled && (url === "/api/config" || url.startsWith("/api/config?")) && !auth.getUser(req)) {
  ...
  if (isMediaRequest) {
    ...
    if (req.method === "POST" && url === "/api/media/upload") {
      ...
      const asset = await saveMediaAsset(Buffer.concat(chunks), String(req.headers["content-type"] ?? ""), String(req.headers["x-filename"] ?? "upload"));
    ...
    if (req.method === "GET") {
      ...
      res.writeHead(200, { "Content-Type": asset.mimeType, "Cache-Control": "private, max-age=3600" });
      res.end(asset.data);
      return;
    }
  }
