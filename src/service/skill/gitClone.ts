...
const AUTH_FAIL = /Authentication failed|could not read Username|terminal prompts disabled|Permission denied|Invalid username or token|forbidden/i;

/** 代理/网络失败特征串（命中则抛「代理连接失败：」前缀错误，便于前端分类）。 */
const PROXY_FAIL = /proxy|Connection timed out|Failed to connect to|Could not resolve host|ETIMEDOUT/i;
...
export async function listRemoteBranches(
  ...
  try {
    ...
    try {
      const { stdout: sym } = await git(["ls-remote", "--symref", url, "HEAD"], { env: authEnv(auth, proxy) });
    ...
    const stderr = (err && typeof err === "object" && "stderr" in err ? (err as { stderr?: unknown }).stderr : "") ?? "";
    if (AUTH_FAIL.test(String(stderr))) {
      return { branches: [], defaultBranch: undefined, needsAuth: true };
    }
    if (PROXY_FAIL.test(String(stderr))) {
      throw new Error(`代理连接失败：无法通过代理连接仓库，请检查代理地址或网络（${truncateStderr(stderr)}）`);
    }
