 ...
 */
import { existsSync, readdirSync, readFileSync, writeFileSync, renameSync, rmSync, mkdirSync, statSync, type Dirent } from "fs";
...
export function extractZipBuffer(buf: Buffer, destDir: string): void {
  ...
  for (const entry of entries) {
    ...
    if (!resolve(target).startsWith(resolve(destDir) + "/") && resolve(target) !== resolve(destDir)) {
...
export function peekSkillMeta(folder: string): { name?: string; description?: string; trigger?: string } {
  try {
  ...
  if (variants.includes(SKILL_FILE_NAME)) {
    for (const f of variants) {
      if (f !== SKILL_FILE_NAME) {
        try { rmSync(join(folder, f), { force: true }); } catch { /* ignore */ }
      }
    }
    return;
  }
