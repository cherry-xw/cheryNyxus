...
import {
  Method,
  type SkillsListSourcesResponseData, type SkillSourceDTO, type SkillSourceEntry,
  type SkillsCheckSourceRequestData, type SkillsCheckSourceResponseData,
  type SkillsCheckAllSourcesRequestData, type SkillsCheckAllSourcesResponseData,
  type SkillsResyncSourceRequestData, type SkillsResyncSourceResponseData,
  type SkillsDeleteSourceRequestData, type SkillsDeleteSourceResponseData,
  type SkillsResyncAllSourcesRequestData, type SkillsResyncAllSourcesResponseData,
} from "../message/types.js";
import { skillsDir, createStaging, removeStaging, removeCherySubdir, normalizeSkillFileName } from "./importShared.js";
...
export async function handleSkillsCheckSource(
  ...
  try {
    ...
    updateSourceCheck(sourceId, { lastCheckedAt: new Date().toISOString(), lastCheckError: message });
    throw error;
  }
}
...
export async function handleSkillsCheckAllSources(
  ...
  for (const source of sources) {
    try {
      ...
      failed.push({ sourceId: source.id, reason: error instanceof Error ? error.message : String(error) });
    }
  }
...
export async function handleSkillsResyncAllSources(
  ...
  for (const entry of sources) {
    try {
      ...
      results.push({ sourceId: entry.id, ok: true, commitSha: clone.commitSha, commitDate: clone.commitDate });
      successes++;
    } catch (err) {
...
export function registerSkillSourceHandlers(router: import("../message/router.js").RpcRouter): void {
