...
function parseQuestionArguments(argumentsJson: string): Omit<QuestionBatchItemInput, "questionId" | "createdAt"> | null {
...
function insertQuestionBatch(
  ...
  const batch = db.prepare(
    "SELECT status FROM question_batches WHERE batch_id = ? AND chat_id = ?",
...
export function backfillLegacyPendingQuestionBatches(chatId: string): void {
  ...
  const migrated = db.prepare(
    "SELECT legacy_backfill_version FROM question_projection_meta WHERE chat_id = ?",
  ...
  const backfill = db.transaction(() => {
    ...
    for (const assistant of messages) {
      if (assistant.role !== "assistant" || assistant.revoked !== 0 || !assistant.sense_calls) continue;
      const exists = db.prepare(
        "SELECT 1 FROM question_batches WHERE batch_id = ? AND chat_id = ?",
      ...
      for (const call of calls) {
        if (call.name !== "ask_user_question" || typeof call.id !== "string" || typeof call.arguments !== "string") continue;
...
function readPendingQuestionBatches(db: ReturnType<typeof getMonthlyDb>, chatId: string): PendingQuestionBatchSnapshot[] {
...
export function hasPendingQuestionBatches(chatId: string): boolean {
  ...
  return Boolean(db.prepare(
    "SELECT 1 FROM question_batches WHERE chat_id = ? AND status = 'pending' LIMIT 1",
  ).get(chatId));
}

function normalizeAnswer(item: QuestionItemRow, answer: QuestionBatchAnswerInput): {
  answerText: string;
  answerJson: string;
...
export function completeQuestionBatch(
  ...
  const complete = db.transaction((): CompletedQuestionBatch => {
    const batch = db.prepare(
      "SELECT * FROM question_batches WHERE batch_id = ? AND chat_id = ?",
    ...
    for (const answer of answers) {
      if (answerById.has(answer.questionId)) throw new Error(`Duplicate answer for question "${answer.questionId}"`);
      answerById.set(answer.questionId, answer);
    }
    if (answerById.size !== items.length || items.some((item) => !answerById.has(item.question_id))) {
    ...
    const completedAnswers = items.map((item) => {
      ...
      const messageUpdate = db.prepare(
        "UPDATE messages SET content = ? WHERE id = ? AND chat_id = ? AND role = 'sense'",
      ).run(normalized.answerText, item.question_id, chatId);
      if (messageUpdate.changes !== 1) {
        throw new Error(`Sense message "${item.question_id}" was not found for question batch "${batchId}"`);
      }
      ...
      ).run(normalized.status, normalized.answerJson, normalized.answerText, completedAt, item.question_id, batchId);
      if (itemUpdate.changes !== 1) throw new Error(`Question "${item.question_id}" is no longer pending`);
      return {
        questionId: item.question_id,
    ...
    if (batchUpdate.changes !== 1) throw new Error(`Question batch "${batchId}" is no longer pending`);
