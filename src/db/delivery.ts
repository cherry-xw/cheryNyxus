...
export function claimRequest(requestId: string, method: string, params: unknown): RequestClaim {
  ...
  if (existing.method !== method || existing.params_hash !== fingerprint) return { state: "mismatch" };
...
export function appendChatEvent(chatId: string, event: Record<string, unknown>): number {
  ...
  const write = db.transaction(() => {
    ...
    db.prepare("UPDATE chat_events SET event_json = ? WHERE rowid = ?").run(JSON.stringify(stored), inserted.lastInsertRowid);
    db.prepare("DELETE FROM chat_events WHERE chat_id = ? AND created_at < ?")
      .run(chatId, now - RETENTION_MS);
    ...
    if (cutoff) db.prepare("DELETE FROM chat_events WHERE chat_id = ? AND chat_seq <= ?").run(chatId, cutoff.seq);
    return seq;
  });
  return write();
}
...
export function createSpawnTask(input: Omit<SpawnTask, "taskId" | "status"> & { taskId?: string }): SpawnTask {
  ...
  getSoulDb().prepare(
    `INSERT INTO spawn_tasks
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
  ).run(taskId, input.childChatId, input.parentChatId, input.type, input.prompt, input.brain, input.senseGroup, input.wait ? 1 : 0, now, now);
...
export function getSpawnTask(taskId: string): SpawnTask | undefined {
  const row = getSoulDb().prepare("SELECT * FROM spawn_tasks WHERE task_id = ?").get(taskId) as Record<string, unknown> | undefined;
  return row ? toSpawnTask(row) : undefined;
}

export function getSpawnTaskByChild(childChatId: string): SpawnTask | undefined {
  const row = getSoulDb().prepare("SELECT * FROM spawn_tasks WHERE child_chat_id = ?").get(childChatId) as Record<string, unknown> | undefined;
  return row ? toSpawnTask(row) : undefined;
}
...
export function finishSpawnTask(taskId: string): void {
  getSoulDb().prepare(
    "UPDATE spawn_tasks SET status = 'finished', updated_at = ? WHERE task_id = ?",
  ).run(Date.now(), taskId);
}
