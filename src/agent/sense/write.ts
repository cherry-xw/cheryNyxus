...
const WriteSchema = z.object({
  ...
  offset: z.number().int().min(0).describe("起始行号偏移量（0-based），与 limit 配合使用实现行范围替换。不指定则写入整个文件").optional(),
  limit: z.number().int().min(0).describe("要替换的行数，与 offset 配合使用。不指定则写入整个文件").optional(),
});
