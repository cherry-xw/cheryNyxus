 ...
 * 截断大文件（仅保留头部）
 */
function truncateContent(content: string, previewLines: number, filePath: string): string {
  ...
  const result = headLines.map((line, index) => `${index + 1}\t${line}`).join('\n')
...
export default sense(
  ...
  async (input, senseSharedData: SenseSharedData): Promise<SenseResult> => {
    try {
      ...
      const logExtensions = compressionConfig?.log_file_extensions ?? [
        '.log',
        '.txt',
        '.out',
        '.err',
      ...
      // 生成基础hash并写入senseSharedData（用于write修改检测）
      ...
      let readNamespace = senseSharedData.get('read_file')
      if (!readNamespace) {
        readNamespace = new Map()
        senseSharedData.set('read_file', readNamespace)
      }
