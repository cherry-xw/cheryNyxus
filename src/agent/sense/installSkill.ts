...
import {
  createStaging, removeStaging, extractZipBuffer,
  skillsDir, stagingRoot, normalizeSkillFileName, parseGithubUrl, NAME_PATTERN,
...
const stageSchema = z.object({
  ...
  url: z.string().describe("技能来源 URL：zip 直链 / git 仓库（https|git@|ssh）/ manifest（YAML frontmatter 含 source）"),
 ...
 */
async function doStage(url: string, branch: string | undefined, depth: number): Promise<SenseResult> {
  if (depth > 1) throw new Error("manifest 嵌套层数超限（source 指向的仍是 manifest？请指向 zip/git/文件）");
  ...
  try {
    if (looksGit && gitParsed) {
      ...
      if (isZip(buf)) {
        ...
        if (!/^---\r?\n[\s\S]*?\r?\n---/.test(text)) {
          throw new Error("来源内容无法识别：非 zip / 非 git URL / 非 manifest（frontmatter 缺 source）/ 非 SKILL.md（缺 frontmatter）");
        }
    ...
    return {
      content: `已获取来源 ${url}，发现 ${result.candidates.length} 个候选技能：\n` +
        JSON.stringify({ stagingId: result.stagingId, candidates: result.candidates, sourceUrl: url }, null, 2) +
...
function readStagingManifest(stagingId: string): SkillManifest {
  ...
  if (!existsSync(p)) throw new Error(`暂存 manifest 不存在（stagingId=${stagingId}），请重新 stage`);
  return JSON.parse(readFileSync(p, "utf-8")) as SkillManifest;
}

/** commit：据 selections cpSync 到 .chery/skills/ + 规范化 + 落追溯 + 清 staging。 */
function doCommit(args: { stagingId: string; selections: Array<{ name: string; import: boolean }> }): SenseResult {
  ...
  return {
    content: `安装完成：${imported.length ? imported.join(", ") : "（未选任何项）"}` +
