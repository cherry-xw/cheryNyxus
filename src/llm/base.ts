...
export abstract class BaseLLMClient<TConfig extends ClientConfigBase> {
  ...
  protected _processToolCallDelta(
    ...
    const deltas = this.message.getAdapter().extractStreamToolCallDeltas?.(chunk) ?? [];
    for (const delta of deltas) {
      const id = this.tool.getAdapter().getToolCallDeltaId(delta);
      if (id && !accumulated.has(id)) {
        accumulated.set(id, { id, name: "", arguments: "" });
      }
      if (id) {
        const data = accumulated.get(id)!;
        const name = this.tool.getAdapter().getToolCallDeltaName(delta);
        if (name) data.name = name;
        const args = this.tool.getAdapter().getToolCallDeltaArguments(delta);
  ...
  async send(threadId: string, input: string): Promise<SendResult> {
    ...
    const messages = this.message.getAdapter().buildMessages(history);
    const tools = this.tool.getAll().length > 0 ? this.tool.getAdapter().buildTools(this.tool.getAll()) : [];
    const options = this.config.thinking ? { thinking: true } : undefined;

    let response = await this.chat(messages, tools, options);

    let toolCalls = this.message.getAdapter().extractToolCalls?.(response) ?? [];
    while (toolCalls.length > 0) {
      const content = this.message.getAdapter().content(response);
      messages.push(this.tool.getAdapter().buildToolCallMessage(content, toolCalls));

      for (const tc of toolCalls) {
        const name = this.tool.getAdapter().getToolCallName(tc);
        const args = this.tool.getAdapter().parseToolCallArguments(tc);
        const id = this.tool.getAdapter().getToolCallId(tc);
        ...
        if (toolDef && toolDef.supervisionLevel <= autoLevel) {
          ...
          messages.push(this.tool.getAdapter().buildToolResponseMessage(id, result));
        } else {
      ...
      toolCalls = this.message.getAdapter().extractToolCalls?.(response) ?? [];
    }

    const finalContent = this.message.getAdapter().content(response);
    const finalThinking = this.message.getAdapter().thinking?.(response);
    const llmres = this.message.getAdapter().wrapFinalResponse(
      threadId,
      finalContent,
      finalThinking,
      response,
  ...
  async confirmToolCall(approved: boolean): Promise<SendResult> {
    ...
    if (!approved) {
      messages.push(
        this.tool.getAdapter().buildToolResponseMessage(toolCallId, "用户拒绝执行该操作"),
      );
    ...
    messages.push(this.tool.getAdapter().buildToolResponseMessage(toolCallId, result));
  ...
  async *sendStream(
    ...
    for await (const chunk of stream) {
      const delta = this.message.getAdapter().extractStreamDelta(chunk);
      accumulated += delta;

      const thinkingDelta = this.message.getAdapter().extractStreamThinking?.(chunk) ?? "";
    ...
    if (toolCallsAccumulated.size > 0) {
      ...
      messages.push(this.tool.getAdapter().buildToolCallMessage(accumulated, toolCalls));
    ...
    const finalResponse = this.message.getAdapter().wrapFinalResponse(
      threadId,
      accumulated,
      thinkingAccumulated,
      null,
