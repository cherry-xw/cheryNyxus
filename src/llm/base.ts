import type { ClientConfigBase, SendResult, ToolCallAccumulator } from "./types";
import type { LLMResponse, LLMStreamChunk } from "@/message/messageFactory";
import { SupervisionLevel } from "./types";
import { createUserMessage, accumulateMessages, getAdapter } from "@/message/index";
import { ToolManager } from "@/tool/base/toolManager";
...
export abstract class BaseLLMClient<TConfig extends ClientConfigBase> {
  ...
  protected adapter!: ReturnType<typeof getAdapter>;
  /** 消息历史快照（用于两阶段执行恢复） */
  ...
  constructor(sessionId: string, config: TConfig) {
    ...
    this.adapter = getAdapter(config.provider);
    if (!this.adapter) {
   ...
   * 完成工具调用累积
   */
  protected _finalizeToolCalls(accumulated: Map<string, ToolCallAccumulator>): unknown[] {
  ...
  async send(threadId: string, input: string): Promise<SendResult> {
    const history = accumulateMessages(this.sessionId, createUserMessage(threadId, input));
    ...
    while (toolCalls.length > 0) {
      ...
      for (const tc of toolCalls) {
        ...
        const autoLevel = this.config.autoExecuteLevel ?? SupervisionLevel.confirm;
        ...
        if (toolDef && toolDef.supervisionLevel <= autoLevel) {
          ...
          this.pendingToolCall = { toolCallId: id, toolName: name, args, threadId };
    ...
    const llmres = this.adapter.wrapFinalResponse(threadId, finalContent, finalThinking, response);
  ...
  async confirmToolCall(approved: boolean): Promise<SendResult> {
    ...
    if (!approved) {
      messages.push(this.tool.buildToolResponseMessage(toolCallId, "用户拒绝执行该操作"));
  ...
  async *sendStream(threadId: string, input: string): AsyncGenerator<LLMStreamChunk<unknown>> {
    const history = accumulateMessages(this.sessionId, createUserMessage(threadId, input));
    const messages = this._buildMessages(history);
    const tools = this.tool.getAll().length > 0 ? this.tool.buildTools() : [];
    const options = this.config.thinking ? { thinking: true } : undefined;
    ...
    const finalResponse = this.adapter.wrapFinalResponse(threadId, accumulated, thinkingAccumulated, null);
