...
/** contextLimit 兜底值：brain 未配 contextLimit 时使用 */
 ...
 * 计算 chat 上下文用量比例（0-1）。
 *
 * limit 来源：chat 持久化 runtime 的 brain 对应 config.llm.brain[brain].contextLimit；
 * brain 未配或缺失 → DEFAULT_CONTEXT_LIMIT 兜底。
