...
export async function startWorker(args: string[] = process.argv.slice(2)): Promise<void> {
  ...
  if (reconcileResult.fixed > 0) {
    logger.event("db.reconcile", { checked: reconcileResult.checked, fixed: reconcileResult.fixed }, LogLevel.warn);
  }
...
async function compileSensesCommand(): Promise<void> {
  ...
  await reloadSenses();
  if (summary.failed.length > 0 || [...testResults.values()].some((r) => !r.detail.passed && r.detail.error)) {
    process.exitCode = 1;
  }
}
