...
用户于 2026-09-20 明确允许 `relay-gateway` 计划新增并运行安全与协议自动测试。例外仅覆盖：

- 新增的 relay、认证边界、动态发现、管理器、子路径和 Electron 解耦测试；
- 为运行上述测试必须直接调整的现有测试入口；
- 与本计划修改文件直接对应的既有定向套件。

本例外不授权整理、修复或全量运行无关旧测试，也不改变下述预存失败基线。每个计划小任务只运行其登记的定向命令，最终全量范围在综合验证阶段按最新实现重建。
