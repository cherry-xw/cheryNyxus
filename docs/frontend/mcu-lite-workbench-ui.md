...
- 审批 / 提问交互以**浏览器式多页签**呈现在输入区上方的待处理面板内（v0.4.2）：面板为**独立直角卡片**（直角边框，无圆角——用户强制规则）。**分色硬约束（用户多次强调）**：**标签栏位置（标签栏容器背景）永远无底色**——面板容器透明、无边框，边框由 bar 底线（标签栏下一行）+ 内容区左/右/下三边承担（紫 45%）；紫色底色（v1.7b 起，`--violet` token 派生；原琥珀被用户判定「显脏、不贴合主题」废弃）只加在**内容元素**上：①**tab 按钮自身**有底色（v1.7，恢复 v0.4.2 浏览器式连体）——非激活中性亮底 `--el-bg-color` + 略矮，激活 = 内容区紫底连体 + 主色边框文字 + 略高盖过非激活（底部 padding 差），激活不收缩、其余空间不足时非激活互相挤压、最短只剩 icon；②**提问内容区** = 紫淡底 20% 派生（题干区透出紫底，**选项卡片同族淡紫底 9% 派生** + 常规边框——v1.7b 用户要求调整选项底色，原中性白底在浅色淡底上边界不清，现为「内容区 20% → 卡片 9%」层次，选中态主色边框 + 主色淡底不变；底部操作栏同家族 26% 更深档 + 紫顶线）；③**倒计时徽章**空白底 + 紫描边（浅色下可读）。多 tab 靠最左依次排列，右端为收起按钮预留空间。
- **面板收起/展开（v1.1，v1.2 重做）**：**收起/展开按钮为带边框 24×24 方形按钮**（直角、字重 400，hover 主色高亮），**绝对定位挂在面板右上角、相对标签栏垂直居中**（展开态显示 ▲=收起，收起态显示 ▼=展开）；**收起态面板仅保留标签栏**（内容区高度过渡 200ms 收到 0，`prefers-reduced-motion` 直切；隐藏后 `visibility: hidden` 防止焦点落入），直接贴在输入区上方；收起态点击任意标签 = 展开并切换到该交互；新审批/提问到达**不自动展开**（保持收起，激活 tab 倒计时徽章照常提示）；收起状态按窗口 × 根会话隔离持久于 lite store（`pendingCollapsed`），切会话互不影响。**工作台右侧铃铛在精简模式下与面板内 ▲/▼ 同语义**：点击切换该面板收起/展开（角标显示当前根待处理数量），只折叠/展开面板本身，不隐藏 lite 视图。
...
- question_batch_requested（提问）：**全量下发**（题干 ≤500B、选项标签 ≤60B）→ 渲染选择题，提交 → interaction.question.answer。
  - **多选题（v0.2 补）**：wire 的 question 字段含 `multiSelect` 标志——单选/多选只决定选择行为；提交时 selectedLabels 数组去重，服务端白名单校验；多选题数上限以服务端校验为准，UI 不自行限制。
  - **选项区（v1.5 重做，v1.6 精简）**：选项以**两列卡片**呈现（窄屏回落单列），不再整行通栏；每个选项卡片内带「补充」按钮——**仅该选项选中后可用**，点击展开该选项的补充输入框（取消选中即收起并丢弃补充内容）。题末常驻「其他」卡片，其输入框本身作为**单选/多选的一个选项**：单选点击（圆形 radio 标记）即抢走其他选项的选中态、再点可取消，输入内容自动激活并清空其他选中；多选需手动勾选（方形复选框标记），输入内容自动勾选、清空内容即取消勾选。单选/多选均支持再次点击已选项取消选中。**v1.6：**「其他」卡片不再显示「其他 / 用自己的话补充回答」文字描述——radio 标记后直接单行输入框（高度与左侧选项卡片一致），点击输入框不触发选项切换，输入即激活/勾选；「单选 · 再点已选项可取消」说明行删除（radio 交互足够直观），多选/自由回答提示保留。
  - **多问题切换（v1.5 重做）**：批次内多题切换由面板底部操作栏左侧分页器（上一题 n/N 下一题）承接，右侧为提交按钮（全部答完才可提交）——原中间左侧问题导航列表移除。
  - **批次 tab（v1.5）**：待处理面板 tab 即批次：同一 root 下多 agent 并存时 tab 前缀 Agent 名区分归属；同一 agent 的多个提问批次显示问题批次号（如「提问批次 1」），tooltip 含题数。
