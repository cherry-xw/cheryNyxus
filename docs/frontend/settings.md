...
连接配置只保存服务、API 协议和模型等最终值。全部具体模型规则保存在用户可编辑的 `.chery/model-catalog.yaml`，程序代码不隐藏厂商规则。模型目录根据精确/glob/正则规则展示识别结果，并在选择模型时按当前 API 协议从 `recommendByProtocol` 读取协议专属的协议、上下文、thinking 和能力推荐；没有协议专属值时回退到 `recommend`。推荐可带 `provider`：切换模型时自动把服务与协议切到推荐值（服务切到官方厂商入口时地址按该服务默认地址联动）；推荐未给出服务/协议时不重置当前选择。切换协议只更新仍等于旧自动推荐的字段，用户手动修改过的值不会被静默覆盖。未命中时写入 `unknown.recommend` 的保守草稿，用户保存后才生效。
