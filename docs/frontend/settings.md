...
**多媒体 Tab 的 env 入口**：媒体服务的密钥字段与大脑 Tab 同机制——下拉选项来自 `.env` 变量名，标签行提供「打开 .env」与「刷新密钥列表」按钮（复用大脑卡片的 `utils.openFile` + `env.list` 模式），编辑 `.env` 后刷新即可见新变量。外部服务密钥统一写 `.env`（`$ENV` 占位），sense 执行时由后端展开，不硬编码进工具代码。
