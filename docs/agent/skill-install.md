# AI 安装技能（install_skill 感官 + 管家角色 + 路径守卫）

> 源码 [src/agent/sense/installSkill.ts](../../src/agent/sense/installSkill.ts) ｜ [src/utils/pathGuard.ts](../../src/utils/pathGuard.ts) ｜ [src/agent/middleware/tool.ts](../../src/agent/middleware/tool.ts) ｜ 上级 [agent](./README.md) ｜ 相关 [./plugin.md](./plugin.md)（人工导入对照）、[./prompt.md](./prompt.md)（默认 Skill）、[../core/sense.md](../core/sense.md)（感官注册）、[./middleware.md](./middleware.md)（守卫在 tool middleware）

## 职责

让 AI 自行安装技能——用户对主 agent 说「根据 `<url>` 安装技能」，主 agent 通过默认 Skill 引导 `spawn_role` 派出**管家角色**（housekeeper），管家调用 `install_skill` 感官完成 fetch → `/tmp` 暂存 → 候选确认 → 落盘 `.chery/skills/`。

与 [plugin.md](./plugin.md) 的人工 settings 导入对比：

| 维度 | 人工导入（skills.importUrl / plugins.*） | AI 安装（install_skill） |
|------|------------------------------------------|--------------------------|
| 触发 | 前端 settings UI 人工操作 | 主 agent 自然语言 → spawn 管家 |
| 执行者 | RPC handler（前端驱动） | install_skill 感官（管家角色专用） |
...
① 默认 Skill（install-skill）          ← 主 agent 注入，识别安装意图
   └─ 引导 spawn_role({type:"housekeeper", wait:true})

② 管家角色（housekeeper）               ← 专用角色，senseGroup 隔离
   └─ senseGroup: [install_skill, read_file, search_codebase, ask_user_question]
...
fetch(url)
  ...
  │         私有仓 needsAuth → 返错误，管家 ask_user 取凭据后重试
...
管家 stage 后调 `ask_user_question`（逐项选装 / conflict 确认）→ `ctx.yieldTurn()` 释放 turn → 用户答题（service `resolveQuestionBatch` 回灌）→ 管家据答案调 commit。仿 [ask.ts](../../src/agent/sense/ask.ts) / spawn.ts wait=true 的 yield-turn 模式（详见 [middleware.md 问答流程 C](./middleware.md)）。

## 路径守卫（[pathGuard.ts](../../src/utils/pathGuard.ts) + [tool.ts](../../src/agent/middleware/tool.ts)）

### 动机

通用感官 `write_file`/`read_file`/`execute_command` 无路径范围校验（[探索结论](./middleware.md)），能任意读写 `.chery/`。需隔离：仅管家通过 install_skill 可改 `.chery/`，其他角色禁操作。
...
**拦截文案**：`".chery/ 是系统配置目录（技能/插件/提示词/命令/数据库），不能直接读写。安装或修改技能请用 spawn_role 派出「管家」角色，通过 install_skill 感官完成。"`
...
守卫按 `.chery` 路径段匹配，天然覆盖 `.chery/` 下所有子目录（skills/plugins/prompt/command/db/memory/secrets）。未来「修改提示词」「改命令」等设置类操作同理命中，均需走专用感官 + 专用角色。
...
roles:
  housekeeper:
    ...
    lock: true                  # 锁定禁止前端删除（保护 install 能力，RolesTab 显示 lock 图标）
presets:
  默认:
...
frontmatter `name/description/trigger`（trigger = 「用户请求安装/导入技能、skill 包、商店时」）。正文：识别安装意图 → `spawn_role({type:"housekeeper", prompt:<需求>, wait:true})` → 等管家回报。主 agent 不直接装（无 install_skill 感官，被 senseGroup 隔离）。
...
| [.chery.template/prompt/housekeeper/housekeeper.md](../../.chery.template/prompt/housekeeper/housekeeper.md) | 管家 persona |
| [.chery.template/config.yaml](../../.chery.template/config.yaml) | sense_groups.housekeeper + roles.housekeeper + presets.默认.roles |
...
未来「修改 prompt」「改 command」等：新增专用感官（如 `edit_prompt`）+ 加入管家 senseGroup + 加入 `GUARD_EXEMPT`。守卫按 `.chery` 路径段已覆盖这些目录，无需改正则。
