...
- **副作用编排**（observer.ts）：统一消费 agent 内部 effect chunk（`message_created` / `message_updated` / `sense_pending`），触发 DB 写（`addMessage` / `fillApprovalResult` / `markMessageReplaced`）与审批注册（`approvalManager.register`），abort 时 flush 兜底保证 DB 一致。
- **协议映射**（streamMapper.ts）：`MiddlewareChunk` → 协议 `Chunk` / `Notification`（`sense_end`→`interrupt`、`sense_accept/reject`→`accept/rejected`、`message_updated` 带 replace → `replaced` 等）。
