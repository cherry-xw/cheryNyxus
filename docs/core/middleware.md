...
`core/middleware/` 是 Agent 执行链的核心：把多个中间件按洋葱模型组合成一条 generator 链，再由 `Middleware` 类承载**单个 chat 的可变状态**（chatId、消息历史、用户输入队列、运行时配置），对外暴露 `init` / `configureRuntime` / `send` / `abort` / `revokeTrailingCycle` 等门面方法。

它是洋葱模型的引擎，但**不定义中间件的具体行为**——`checkpointMiddleware`、`senseMiddleware`、`retryMiddleware`、`chatMiddleware`、`loopHandler` 都在 [`agent/middleware/`](../../src/agent/middleware/) 实现，通过 `Middleware` 构造参数注入。core 只定义契约（`MiddlewareHandler` / `LoopHandler` / `MiddlewareContext` / 全部 Chunk 类型）和组合机制（`compose`）。
...
| [index.ts](../../src/core/middleware/index.ts) | `Middleware` 类（默认导出，单 chat 绑定，状态机门面） |

> ⚠ CLAUDE.md 旧描述提到「`chatMap` / `createChat` / `send`」顶层导出，**真实代码不存在**。Chat 多路复用与缓存发生在 [`service/chat/`](../../src/service/chat/)；core 层的 `Middleware` 类是**单 chat** 的，service 层为每个 chatId 创建独立实例。
...
- **单 chat 绑定**：一个 `Middleware` 实例 = 一个 chatId，service 层每 chat 创建独立实例，跨轮不重建。
...
- **被依赖**：
  - [`agent/middleware/index.ts`](../../src/agent/middleware/index.ts) —— `new Middleware(...)` 构造、`defaultHandlers` / `createLoopHandler` 注入。
  - [`agent/builder.ts`](../../src/agent/builder.ts) —— `AgentBuilder` 全部门面方法转发给 `Middleware` 实例。
...
3. 若需在 loop 外层加策略（如自定义收敛条件），实现 `LoopHandler` 并传给 `Middleware` 构造的第三参数（默认走 `createLoopHandler(maxLoopCount)`）。
4. core/ 本身无需改动——`compose` 与 `Middleware` 与具体 handler 无关。
