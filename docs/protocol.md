...
| `chat.epoch.list`                               | 列出根会话的不可变上下文纪元。params `{chatId}`；返回 `activeEpochId?` 与 `epochs[]`。只有 `executable=true` 的活动纪元可运行，归档/退役历史全部只读。 | 否   |
| `chat.promptSnapshot`                           | 查看当前或指定历史纪元冻结的系统提示词和工具契约。params `{chatId, epochId?}`；返回纪元状态与快照质量。缺失精确历史快照时明确返回 partial/reconstructed 警告，不用最新配置伪造。 | 否   |
...
| `chat.delete`                                   | 删除聊天（目标为主 chat 即无 `parent_chat_id` 时，级联删其所有子 chat + 各自消息 + 清内存 runtime；若该主 chat 属分支链路 `conversation_branches.task_id`，则同 task 下所有分支根会话连同各自子 chat 一并级联删除，杜绝孤儿分支根）；响应返回 `{chatId,deletedChatIds}`，后者是实际删除目标及全部后代（含分支链路）的权威集合。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 否   |
