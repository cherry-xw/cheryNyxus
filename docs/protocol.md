...
| `utils.models` | 独立工具：基于用户提供的 `{provider, url, key?}` 拉取可用模型列表。`provider` 支持 `openai`/`ollama`；`url` 必填（API base URL）；`key` 可选（ollama 通常无需，openai 缺 key 或占位符 `$VAR` 未替换时返回中文友好 `error` 提示本地 LM Studio/vLLM 等不校验 key 的服务可填任意非空字符串，不抛 RpcError）。返回 `{models: [{id, name?, ownedBy?}], error?}`；请求失败时 `models` 为空数组、`error` 携带错误信息（非 RpcError，前端可展示）。后续该模块会扩展其他便捷信息查询工具 | 否 |
