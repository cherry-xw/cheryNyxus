...
| `generate.image/video/audio` | `false` | 是否可调用对应媒体生成能力（兼容期保留标记，不再作注入门，见下） |

兼容策略：旧 brain 未配置 `capabilities` 时仍视为支持 Tool Call，媒体能力均关闭。`generate.*=true` 时不得设置 `toolCall=false`。

## 生成类工具：自定义 sense 承担（旧内置生成 sense 已删除）

过去内置的 `generate_image` / `generate_video` / `generate_audio` sense 与 `config.media` 媒体网关链路已在破坏性收尾中整体删除（2026-09-22）。现在**生成/理解类媒体能力统一由自定义 sense 承担**：在 `.chery/senses/` 编写或让 CherryNexus 生成外部服务工具（见 [.chery.template/docs/senses.md](../../../.chery.template/docs/senses.md) 与 [sense.md](./sense.md)），工具声明 `capabilities.produces` / `accepts` / `preprocess`，密钥走 `.env`，感官组配置即注入。

- **注入不再受大脑 `capabilities.generate.*` 双门限制**：感官组配置了对应工具即注入；`generate.*` 标记仅作兼容期保留（config 校验仍要求其与 Tool Call 兼容），不再作为注入门。
- 工具按其自身 `supervisionLevel` 执行（写文件 / 执行命令等破坏性操作由规则表判定为需确认）。
...
上传资产保存在 `.chery/media/`，文件名为 UUID 加原始安全扩展名。允许的 MIME 类型为 PNG/JPEG/WebP/GIF、MP4/WebM/MOV、MP3/WAV/OGG/M4A/WebM；单文件大小由全局上传上限控制，缺省 100 MiB。
...
发送弹窗仅允许上传当前临时 brain 的 `input.*` 已开启类型或感官组工具 `accepts` 命中的类型。上传完成后消息正文追加 `[[media:<filename>]]` 引用；该引用是内部协议，不应手工伪造。

`chatMiddleware` 在构造 provider 消息前解析引用：按脑声明走多模态旁路或前置工具调度（见下表）。旧媒体网关 `understand` 转写路径已随破坏性收尾删除。

### 输入处理（enrichMediaInputs）
...
| `input.*` 全 false 或缺省，且无前置工具 | 原样保留：无工具命中则不处理，`[[media:]]` 标记留在消息中（旧媒体网关 understand 转写路径已删除） | 文本消息 |
...
- **入口**：[src/agent/middleware/chat.ts](../../../src/agent/middleware/chat.ts) `enrichMediaInputs`（多模态旁路 `enrichMediaInputsMultimodal` + 前置调度 `enrichMediaInputsPreprocess`）；adapter 接口见 [src/core/message/adapter.ts](../../../src/core/message/adapter.ts) `LLMAttachment` / `groupAttachmentsByMessage` / `buildMessages(history, attachments?)`。
- **MiniMax 图片协议**：走 `openaiCompat` 的 `image_url` data URI base64 透传；单图≤10MB、请求≤64MB。文生图/图生图等生成能力由自定义 sense 承担（出参归一化后落盘 `/api/media/`）。token 预检可调 MiniMax `POST /v1/responses/input_tokens` 或 Anthropic `count_tokens`（见 3.2）。
...
- **AI 大脑设置**：可编辑 Tool Call、三类输入理解、三类生成/编辑能力标记（兼容期保留）。
- **角色设置**：角色没有能力覆写；根据 brain 动态禁用工具/MCP，并在不兼容时清空配置。
- **发送弹窗**：显示 Tool Call 与媒体徽标；临时换模型即时重新约束工具；提供图片/视频/音频上传入口（感官组工具 `accepts` 命中或 brain `input.*` 开启时可上传）。
- **`brain.list`**：返回 `capabilities`，供前端展示和运行期选择。
- **配置保存**：Zod schema 与 `validateRawConfig` 同时覆盖能力，保存后按既有规则重启生效。

## 验收要点

1. `toolCall:false` 的角色无法保存 sense/MCP；临时换到该模型后请求不携带 `tools`。
2. 生成类工具只要感官组配置即注入（不再依赖大脑 `generate.*` 标记）。
3. 上传不支持的 MIME、超出限制的文件或当前模型/感官组不支持的类别均被拒绝或提示；感官组工具 `accepts` 命中某媒体类型时该类型可上传（由工具前置处理）。
4. 多模态模型走附件直传；非多模态且感官组配置 `preprocess` 工具则优先走前置调度；两者都没有时消息原样保留。
5. 自定义生成 sense 的产物可通过受控 `/api/media/:filename` 读取。
