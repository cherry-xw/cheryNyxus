...
- 卡牌、流程图与阅读器统一以右侧抽屉覆盖节点树（档案同款：遮罩 + 标题栏 + 关闭按钮；画布保持全宽、不压缩），抽屉宽度可拖拽及键盘调整（← 变宽 / → 变窄 / Home / End）：默认 50%、最小 300px 或 24% 容器宽、最大 88%；点遮罩或 ✕ 关闭。2026-09-15 用户要求（左抽屉/分栏于 2026-09-26 调整为档案同款右侧抽屉）。
