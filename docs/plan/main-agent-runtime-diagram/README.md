...
状态：待综合验证。T30 已修复 Vue Flow 视口 ref 被深层解包导致锚点不可见的问题；当前返回 T07 等待用户实机验收。

## 本轮恢复检查点（优先于历史完成摘要）

当前恢复入口为 [T07](07-final-verification.md)。T30 的定向与完整自动验证已完成，真实界面可见性仍由用户实机确认。2026-09-14 用户反馈：模型节点 CRT 不再常驻/空闲占位，仅在有 live turn 时显示；实现与回归见 [T07](07-final-verification.md) 反馈回填槽。后续反馈：实时 CRT 面板归属**大模型响应**节点（原为模型请求），窗口加宽并收敛 markdown 标题字号与段落/列表行距；实现与回归见 [T07](07-final-verification.md)「CRT 归属与排版修正验证记录」。
