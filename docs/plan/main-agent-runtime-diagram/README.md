...
状态：待综合验证。T30 已修复 Vue Flow 视口 ref 被深层解包导致锚点不可见的问题；2026-09-15 已根据用户实机截图压缩运行头部，移除重复的工具调用面板并重排模型、入口与结果链，自动验证完成，当前返回 T07 等待实机视觉确认。

## 本轮恢复检查点（优先于历史完成摘要）

当前恢复入口为 [T07](07-final-verification.md)。T30 的定向与完整自动验证已完成，真实界面可见性仍由用户实机确认。2026-09-14 的 CRT 生命周期、归属和排版修正及协作横排/工具链右移均已登记到 [T07](07-final-verification.md)。2026-09-15 新截图反馈要求移除重复的工具调用面板，将响应分流下置、模型层收高、入口链拉直，并将工具结果—内容记录—继续判断竖直对齐；后续追加左移错误判断以拉直 checkpoint 通道。实现及 256 种展开组合、端口、类型、lint 与构建验证均已完成，等待用户实机视觉确认。
...
| T07 | [综合验证与用户验收](07-final-verification.md) | 4：新关系与全部折叠组合、兼容和真实视觉 | T30 | 进行中 |
...
- 当前卡牌内容、操作、开关、偏好及数据契约保留；T25 按用户最新授权调整外部阅读区宽度及历史按钮命中布局。
