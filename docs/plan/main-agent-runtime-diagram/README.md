...
- Vue Flow 流程图保留完整步骤、回放、缩放、定位和详情能力，但仅在右侧抽屉侧栏按需挂载，与卡牌、阅读器互斥。
- 侧栏状态为 `none | cards | workflow | reader`，每次工作台打开默认为 `none`；任意侧栏下节点树仍保持横向 Signal。
- 卡牌、流程图与阅读器统一以右侧抽屉覆盖节点树（历史对话抽屉同款：遮罩 + 标题栏 + 关闭按钮；画布保持全宽、不压缩），抽屉宽度可拖拽及键盘调整（← 变宽 / → 变窄 / Home / End）：默认 50%、最小 300px 或 24% 容器宽、最大 88%；点遮罩或 ✕ 关闭。2026-09-15 用户要求（左抽屉/分栏于 2026-09-26 调整为历史对话抽屉同款右侧抽屉）。
...
- 结果全文悬浮窗按需挂载，思考/正文切换；点击复用原卡片。阅读区默认560px，可拖拽/键盘调宽，左侧对话模式按钮（2026-09 起 rail「档案」改名）预留独立命中空间；窄窗口整区阅读。五个冻结文件字节未改，阅读器外部布局由本轮用户授权调整。
