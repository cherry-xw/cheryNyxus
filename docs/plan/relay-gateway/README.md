...
| --- | --- | --- | --- | --- |
| D1 | “无需注册、直接连接”的后端身份握手 | 不建立用户注册流程；后端直接使用本地稳定 `backendId` 连接并由中转下发专用配置 | A/B | 必须明确连接凭据、Backend ID 冲突处理、配置下发和撤销语义 |
| D2 | rathole service 映射 | 只映射本服务所需 HTTP/WS 能力 | B | 未定义 service 名称、token、配置生命周期前不能写部署脚本 |
| D3 | 中转列表访问边界 | 列表可被前端读取；真正控制面必须由目标后端认证 | A/E | 决定列表返回字段、隐藏信息和不存在/离线响应 |
| D4 | 远程入口与 loopback 信任边界 | 隧道流量不得继承本地 `127.0.0.1` 管理员豁免 | C | 这是安全阻塞项，必须先隔离专用远程 HTTP/WS 入口或可信来源标记 |
| D5 | OIDC 回调拓扑 | 每个后端独立 Pocket ID Client；回调包含稳定 Backend ID 和可配置路径前缀 | C/E/F | 必须确定回调由后端接收还是中转保持同源转发 |
| D6 | 凭据文件与密码变更关系 | 本地受保护文件可重复查看；不经中转 | D/C | 必须定义首次生成、手动修改、重新生成和文件权限 |
| D7 | 管理器与后端 IPC | 管理器独立托管 39980，可启停后端和隧道 | D | 必须确定跨平台控制协议、权限和停止中的状态语义 |
| D8 | 子路径规则 | 前端、中转、WS、Cookie 和 OIDC 都使用可配置公共前缀 | F/G | 必须明确 nginx 去前缀/保留前缀策略及 URL 拼接 owner |
...
| --- | --- | --- | --- | --- | --- | --- |
| A | 中转协议与控制层 | 未开始 | 5 | 新建跨进程连接协议、列表、路由白名单、状态、限流和审计边界，涉及公共契约 | 需求文档 | `relay/` 控制服务、共享协议、协议测试、D1/D3 |
| B | rathole 连接与动态下发 | 未开始 | 4 | 外部二进制、出站连接、配置下发、断线恢复和资源隔离 | A、D1 | rathole server/client 配置、连接管理、B 阶段测试 |
| C | 后端动态端口与认证安全 | 未开始 | 5 | 修改既有 HTTP/WS/Auth 入口，涉及 loopback 安全、双认证、Cookie、冷却和兼容 | A、B、D4、D5 | 后端远程入口、动态发现、认证安全、C 阶段测试 |
| D | 本地管理器与运行入口 | 未开始 | 5 | 新的常驻控制进程、跨平台进程管理、凭据文件、统计和服务安装 | C、D6、D7 | `39980` 管理器、CLI、systemd、Windows 托盘启动器 |
| E | 前端连接发现与双登录 | 未开始 | 4 | 改变前端连接目标、缓存、重连和登录状态，同时保持旧直连兼容 | A、C、D | 列表/手动 ID、发现适配器、双登录 UI 和状态管理 |
| F | Electron 前端化与子路径适配 | 未开始 | 4 | 删除现有后端生命周期耦合，改造 preload、打包和资源/路径生成 | C、D、E、D8 | 纯前端 Electron、子路径适配、构建检查 |
| G | nginx、发布和操作文档 | 未开始 | 3 | 把多个进程和外部服务整理为可复用部署方式，涉及配置模板和平台说明 | B、D、E、F | nginx 模板、安装脚本、运行指南、配置说明 |
...
- 建立独立管理器进程，固定监听 `127.0.0.1:39980`；
...
- 当前小任务：尚未进入 A 的实施；
- 当前阻塞决策：D1-D8，尤其是直接连接握手、rathole service 映射、远程认证入口和管理器 IPC；
- 工作区约束：存在其他任务的未提交修改，不得 reset、覆盖、批量格式化或混入本任务；
- 下一步：读取计划规范后创建 A 的独立小任务文档，先落定 relay 控制协议和 D1-D3。
