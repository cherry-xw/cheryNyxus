...
胶囊内部使用两行三图标：第一行是会话（历史、新建、会话钢琴），第二行是视图与配置（节点折叠、布局复位、角色）；分组只用行距与细分隔线表达，不产生分组页签、详情面板或二级菜单。图标通过 tooltip 说明功能与状态。钢琴声音与删除属于钢琴局部操作，固定在钢琴面板右上角。执行树节点只负责查看和固定执行详情，不再兼任输入入口。胶囊开合使用裁切揭示；`prefers-reduced-motion` 下取消空间位移，仅保留状态颜色反馈。
