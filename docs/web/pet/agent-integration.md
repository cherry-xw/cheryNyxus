...
- **getHistory action + StreamState.history/historyLoaded/historyDirty**：staged chunk 经 `routeChunk` → `accumulateStaged` 重组为 `HistoryItem[]`（`thinking_end` 开 assistant item / `content_end` 按 role 分流 user|assistant|sense / `sense_end` 挂当前 assistant item.senseCalls；每条带 `agentChatId` 标来源 chatId 供反向溯源）。subagent 检测：role=user 且 content 匹配 `^\[子agent <type>\]` 归类 subagent（item.petName = agentType）。**子 chat 自身重映射**：opened chat 为子 chat（ghost 自身抽屉）时，对自身 history 跑 `remapChildHistory(items, chatId)`（assistant→subagent / user→master，附 `subPetChatId=chatId`）——使首条（主 agent 发的 spawn prompt，DB 存 role:user）显为 master 而非 user；主 chat 自身历史不重映射（保持 user/assistant）。合并主视图时子 chat 历史同样走 `remapChildHistory`（既有逻辑）。**实时镜像**：sendMessage 起始 push user prompt（带临时 msgId，response.userMsgId 到达后替换）；done notification `finalMessage` push 末条 assistant（带 agentChatId）；role_reply push 子 agent 回复到主 chat history（带 agentChatId=childChatId）——drawer 打开期间新 session 消息即时显。**缓存机制**：`historyDirty` 守卫 + `in-flight` Promise 去重（role_created 与 drawer 同时触发共享同一 RPC，避免并发抖动）。**主动预加载**：`initFromChats` 末尾并行预加载 top-5 master + 全部后代 chat；`role_created` notification 触发新子 chat 预加载。
...
- **StreamState.approval / approvalQueue**：
  ...
  - `approvalQueue`：已隐藏但未处理的审批队列（任意长度）。PetIcons 渲染为闪烁 icon，频率 = `max(0.2, min(5, remainingSec * 0.1))` 秒（剩余越少闪得越快）。倒计时归零 → CSS opacity 渐隐消失（`transition 600ms`）。
