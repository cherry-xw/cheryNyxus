...
Nyxus 的常态应读作一个小尺度、风格化的星系入口：受控暖核、蓝紫外盘、沿旋臂局部出现青蓝或玫红恒星形成区、可辨白色恒星核心。参考图用于校正结构、层次和色彩分布，不复制任一张图的构图或调色。

服务端 `disconnected` 时，Nyxus 固定渲染为黑洞/吸积盘；`working` 时固定为脉冲星 Loading；其余时间调度包含单环、多环在内的 idle 形态。Nyxus 是可游走、向鼠标靠近并允许拖拽的弹窗入口，不进入 `PetInstance`；除移动、拖拽和单击开窗外，不提供 Pet 交互或弹窗外业务能力。本文旧阶段 4–6 中的运行细分环系、粒子鼠标手势和 Pet 关联方案不再实施。

## 视觉决定

| 主题 | 决定 | 理由 |
|---|---|---|
| 默认星系 | 主体为正面或轻倾斜的双主臂星系，外段允许少量低密度分叉 | 比六条等权窄臂或均匀彩色云团更适合小尺寸辨识。 |
| 色彩分布 | 中心小范围暖白/浅金；外盘冷蓝靛蓝；玫红、青蓝只作旋臂结 | 接近恒星族群、尘埃与恒星形成区的层次，避免全场霓虹散点。 |
| 恒星 | 核心继续纯白；彩色仅留在极少数恒星光晕、诞生和消亡环 | 延续现有可见性约束。 |
| 双星系 | `merger` 保留伴星系并合循环；`binary` 增加单盘裂变、双核互绕并重新融合的循环 | 两个盘各有自己的小核心，且与单盘之间保持连续因果。 |
...
验收：运行中只显示专属脉冲星 Loading 形态，不再从入口区分思考、工具、等待输入或输出阶段；单环与多环回到 idle 调度。

### 5. 鼠标交互（历史方案，入口重构后不再启用）

Cherry Nyxus 入口保留自主游走、向鼠标缓慢靠近、外层入口拖拽和视口边界约束。粒子内部的 `reach`、释放尾迹、工具环星尘桥、绕圈、掠过、停留和外盘点击均不在入口挂载。
...
### 6. 与其他 Pet 的“关联”效果（取消）

Nyxus 不再作为 Pet，因此不读取 Pet 障碍物或邻近 Pet，不参与避让、关联或引力桥渲染。
...
3. 视觉验收至少覆盖：connected 默认盘、棒旋、倾斜盘、单环、多环、双星与合并阶段、disconnected 黑洞、running 脉冲星 Loading，以及入口自主游走和向鼠标缓慢靠近。
