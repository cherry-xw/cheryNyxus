...
- 一轮回复的收尾答消息（主 agent 最后一条、其后无 tool batch 的可见消息）不计入 fold，始终以普通消息节点展示，避免答案被折叠卡片吞掉。
...
- 节点只渲染 `icon + name + status overlay`。悬浮不得移动、缩放或上下跳动节点本体，只用向外匀速扩散的雷达环反馈；当前显示详情弹窗的节点使用更亮的双环雷达，未显示弹窗的节点保持较弱单环。
