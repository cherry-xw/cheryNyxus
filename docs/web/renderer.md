...
**遮罩实现约束**：**所有全屏 overlay 遮罩**只用纯半透明背景 `var(--scrim)`，**禁止加 `backdrop-filter: blur()`**。适用实例：HistoryDrawer 单蒙层 `.drawer-overlay.is-top-mask`（[HistoryDrawer.vue](../../web/src/features/agent/drawer/HistoryDrawer.vue)）、AgentDialog 模态 `.dialog-overlay.is-top-mask`（[agentDialog.less](../../web/src/features/agent/dialog/agentDialog.less)，`source='history'` 非 pet/nyxus 来源时启用）、SettingsDialog `.settings-overlay`（[SettingsDialog.vue](../../web/src/features/agent/settings/SettingsDialog.vue) 浏览器 overlay 形态；Electron 原生窗 native 变体同样 `backdrop-filter: none`）。`backdrop-filter` 要求每帧对遮罩下方整个内容重新采样做模糊；桌面层 pet 运动循环与 Nyxus 粒子持续 rAF 动画，会让遮罩区域每帧强制重绘（DevTools Paint flashing 持续闪烁、元素持续被标记刷新），即使文字内容毫无变化。视觉上纯色遮罩与带 blur 差异很小。
