...
lite 视图对话流上方的多流水线运行轨迹。**一轴 = 一个 Agent 流程**：主 Agent 一条，每个子 Agent 一条独立流水线；子 Agent 的块始终在其自身流水线上展示，不并入主流水线。

**几何模型（核心）**
- **时间→x 线性映射**：`x = (t − minTime) / span × trackWidth`。均匀刻度依赖线性映射，保证刻度间距与时间跨度成正比（可直接参照缩放比例）。
- **块宽度 = log 压缩 × zoom**：`w(s) = MIN_BAR_PX + 11·ln(1 + s)`（s=秒数，`MIN_BAR_PX = 10`）。1s≈17px、10s≈36px、1h≈100px、1000h≈176px——**亚线性增长有极限**（不随耗时无限变宽）。
- **zoom 应用**：渲染宽 = `clamp(w(s) × zoom, 10, 240)`。放大块变长、缩小块变短；**变短下限 10px**（保证可点击命中，点击关联下方节点高亮）、**变长上限 240px**。缩放范围 0.4x–5x，Ctrl/⌘+滚轮缩放（普通滚轮仍由 LiteScrollbar 横向平移）。
- **等待区间 = 缺口**：块宽再受「到下一个块可用空间」约束（`width = max(10, min(目标宽, nextLeft − left − 1))`）——块与块之间的等待时段是**空余缺口**，不画虚线、不画分割线（删除旧版 dashed wait 块）。

**布局与视觉**
- 块为标准矩形，**无圆角**（`border-radius: 0`）；相邻块之间 **1px 间隔**；类型用颜色区分（user=主色 / model=绿 / tool=黄+工具类型配色），不依赖虚线/边框区分。
- 行标记：每条流水线前缘显示「**序号 · 角色名**」——第一行 · 主 Agent / 第二行 · 子 Agent（角色名），同时满足定位与角色辨识。
- **均匀时间刻度**：底部一行自适应步长的刻度线 + 相对时间标签（首块为 0，后续 0:05 / 0:10…），**只作缩放比例与大跨度参照，不标记节点具体位置**；格式沿用 `trajectoryLabel`（HH:MM:SS / MM:SS）。

**交互**
- 点击块 → 定位并高亮下方对应内容行（含闪光）；hover → 浮层详情（状态/耗时/类型/Agent）；running 块脉冲动画；`data-tooltype` 驱动工具类型配色。
