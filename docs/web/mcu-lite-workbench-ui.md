...
- **链路标签栏**：正文列表顶部（monitor 上方、固定不滚动）常驻一行链路标签——主 Agent（✧ 主 Agent）+ 各子 Agent（◆ 角色名），激活项高亮、点击切换 activeLane，与轨迹行头角色名按钮（§4.11 交互）联动，两处均可切换；标签栏随节点出现增减，用户消息并入主 Agent 链路（§4.11）。
