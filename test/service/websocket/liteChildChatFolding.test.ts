...
describe('T26 子 chat 折叠：服务端语义', () => {
  it('子 done 整帧抑制（最终回复只认 root 维度）', () => {
    expect(
      applyLiteEvent(
        profile,
        notification('done', { finalMessage: { msgId: 'm', content: 'x' } }, CHILD),
      ),
    ).toBeUndefined()
  })
  ...
  it('root done 正常投影（含 finalMessage 截断链路可达）', () => {
    const out = applyLiteEvent(
      ...
      notification(
        'done',
        { canResume: false, finalMessage: { msgId: 'm', content: '回复' } },
        ROOT,
      ),
    )
  ...
  it('子 run.updated 透传（设备侧折叠——服务端不改写，信封 chatId 保留供分流）', () => {
    const out = applyLiteEvent(
      profile,
      notification('run.updated', { runId: 'r-child', status: 'paused' }, CHILD),
    )
    expect(out).toBeDefined()
  ...
  it('子 run.outcome 透传，failed 与 canResume 独立供设备折叠', () => {
    const outcome = {
      status: 'failed',
    ...
    const out = applyLiteEvent(profile, notification('run.outcome', outcome, CHILD))
    expect((out as Notification).data).toEqual(outcome)
    expect((out as { chatId?: string }).chatId).toBe(CHILD)
  })
  ...
  it('子 error 透传且 message 原样（F11；设备按 chatId 折叠为子任务失败态）', () => {
    const out = applyLiteEvent(
      profile,
      notification('error', { message: '[abc12345] 感官出了点小问题', canResume: true }, CHILD),
    )
    expect(out).toBeDefined()
  ...
  it('role_created 投影去 prompt（子任务存在性）+ role_reply 抑制', () => {
    ...
    expect(
      applyLiteEvent(
        ...
        notification(
          'role_reply',
          { parentChatId: ROOT, childChatId: CHILD, type: 't', content: 'c' },
          ROOT,
        ),
      ),
    ).toBeUndefined()
          ...
          operations: [
            {
              ...
              node: {
                id: 'n1',
                kind: 'return',
                actor: { kind: 'agent', roleType: 'researcher' },
...
describe('T26 游标分道（per-chat seq 不可跨 chat 比较）', () => {
  it('子事件与根事件 seq 独立保留（信封 seq 字段不被剥）', () => {
    const rootEvt = applyLiteEvent(profile, {
      kind: 'notification',
      type: 'turn.started',
      chatId: ROOT,
      seq: 5,
    ...
    const childEvt = applyLiteEvent(profile, {
      kind: 'notification',
      type: 'turn.started',
      chatId: CHILD,
      seq: 3,
