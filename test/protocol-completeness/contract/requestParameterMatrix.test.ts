...
const chatMatrices: MethodMatrix[] = [
  ...
  },
  {
    ...
    valid: [
      ...
      value('preset page', { scope: 'preset', presetId: 'preset-1', limit: 24, offset: 0 }),
    ...
    invalid: [
      ...
      value('invalid page limit', { scope: 'preset', presetId: 'preset-1', limit: 0 }),
    ],
  },
  {
    method: Method.CHAT_TASK_LIST,
    valid: [
      value('preset task catalog', { presetId: 'preset-1', limit: 24 }),
      value('snapshot cursor', { cursor: 'cursor-1' }),
    ],
    invalid: [
      value('missing preset and cursor', {}),
      value('inverted time range', { presetId: 'preset-1', updatedFrom: 2, updatedTo: 1 }),
      value('invalid task status', { presetId: 'preset-1', statuses: ['waiting'] }),
    ],
  },
  {
    method: Method.CHAT_TASK_RESULT_VIEW,
    valid: [value('view matching result', { taskKey: 'task-1', resultId: 'result-1' })],
    invalid: [
      value('missing result', { taskKey: 'task-1' }),
      value('empty task key', { taskKey: '', resultId: 'result-1' }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('missing anchor', { rootChatId: 'root-1' }),
      value('wrong anchor type', { rootChatId: 'root-1', anchorNodeId: 1 }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('missing command', { branchId: 'branch-1' }),
      value('empty branch', { branchId: '', commandId: 'command-1' }),
    ],
  },
  {
    ...
    invalid: [
      value('missing task', { commandId: 'command-1' }),
      value('empty command', { taskId: 'task-1', commandId: '' }),
    ],
  },
  {
  ...
  },
  {
    ...
    valid: [
      value('current epoch', { chatId: 'chat-1' }),
      value('historical epoch', { chatId: 'chat-1', epochId: 'epoch-1' }),
    ],
    invalid: [
      value('missing chat', { epochId: 'epoch-1' }),
      value('empty epoch', { chatId: 'chat-1', epochId: '' }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('zero generation', { rootChatId: 'root-1', generationIndex: 0 }),
      value('fractional generation', { rootChatId: 'root-1', generationIndex: 1.5 }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('missing command', { chatId: 'chat-1' }),
      value('empty chat', { chatId: '', commandId: 'command-1' }),
    ],
  },
  {
    method: Method.CHAT_RESUME_TREE,
    valid: [
      value('resume tree', { rootChatId: 'root-1', pauseId: 'pause-1', commandId: 'command-1' }),
    ],
    invalid: [
      value('missing pause', { rootChatId: 'root-1', commandId: 'command-1' }),
      value('empty command', { rootChatId: 'root-1', pauseId: 'pause-1', commandId: '' }),
    ],
  },
  {
    ...
    valid: [
      ...
      value('root tree subscription', {
        scope: 'root',
        rootChatId: 'root-1',
        view: 'tree',
        executionStepLimit: 500,
      }),
    ],
    invalid: [
      ...
      value('step limit above maximum', {
        scope: 'chat',
        chatId: 'chat-1',
        executionStepLimit: 501,
      }),
    ],
  },
  {
    ...
    invalid: [
      value('missing subscription', {}),
      value('empty subscription', { subscriptionId: '' }),
    ],
  },
  {
    ...
    invalid: [
      value('negative completed window', { completedSince: -1 }),
      value('fractional completed window', { completedSince: 1.5 }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('missing child', { rootChatId: 'root-1', commandId: 'command-1' }),
      value('wrong recursive type', {
        rootChatId: 'root-1',
        childChatId: 'child-1',
        commandId: 'command-1',
        recursive: 'yes',
  ...
  },
  {
    ...
    valid: [
      value('abort current run', { chatId: 'chat-1' }),
      value('idempotent targeted abort', {
        chatId: 'chat-1',
        runId: 'run-1',
        commandId: 'command-1',
      }),
    ...
    invalid: [
      value('empty chat', { chatId: '' }),
      value('empty run id', { chatId: 'chat-1', runId: '' }),
      value('empty command', { chatId: 'chat-1', commandId: '' }),
    ],
  },
]
...
const interactionAndConfigurationMatrices: MethodMatrix[] = [
  {
    ...
    valid: [
      ...
      value('bounded window', { presetId: 'preset-1', includeActivity: true, maxItems: 20 }),
    ],
    invalid: [
      value('zero max items', { maxItems: 0 }),
      value('above maximum', { maxItems: 21 }),
      value('empty preset', { presetId: '' }),
    ],
  },
  {
  ...
  },
  {
    ...
    invalid: [
      value('empty brain', { chatId: 'chat-1', brain: '' }),
      value('empty mcp name', { chatId: 'chat-1', brain: 'brain-1', mcpServers: [''] }),
    ],
  },
  {
    method: Method.SESSION_RUNTIME_SET,
    valid: [
      value('primary plus role roster', {
        chatId: 'chat-1',
    ...
    invalid: [
      ...
      value('empty role name', {
        chatId: 'chat-1',
        primary: { brain: 'brain-1' },
        roles: { '': { brain: 'brain-2' } },
      }),
    ],
  },
...
const externalBoundaryMatrices: MethodMatrix[] = [
  {
    ...
    valid: [
      value('minimal complete config', {
        protocolVersion: 2,
        requestId: 'save-1',
        expectedBaseRevision: 'config-1',
        candidate: minimalConfig,
      }),
    ],
    invalid: [
      value('missing global', { llm: minimalConfig.llm }),
      value('invalid supervision enum', {
        ...minimalConfig,
        global: { ...minimalConfig.global, supervision: 'root' },
      }),
  ...
  },
  {
    ...
    invalid: [
      value('short nonce', { sessionId: 'session-1', nonce: 'abc', encPath: '' }),
      value('non-hex nonce', { sessionId: 'session-1', nonce: 'zzzzzzzzzzzzzzzz', encPath: '' }),
      value('unknown field', {
        sessionId: 'session-1',
        nonce: '0123456789abcdef',
        encPath: '',
  ...
  },
  {
    ...
    invalid: [
      value('zero page', { page: 0 }),
      value('fractional page', { page: 1.5 }),
      value('page size above maximum', { page: 1, pageSize: 201 }),
    ],
  },
  {
    method: Method.SKILLS_IMPORT_URL,
    valid: [
      value('credential import', {
    ...
    invalid: [
      value('missing branch', { url: 'https://example.test/repo.git' }),
      value('credential plus password', {
        url: 'https://example.test/repo.git',
        branch: 'main',
        credentialId: 'credential-1',
        password: 'secret',
  ...
  },
  {
    ...
    invalid: [
      value('empty label', { label: '', username: 'git', password: 'secret' }),
      value('missing password', { label: 'GitHub', username: 'git' }),
    ],
  },
  {
...
describe('core request parameter matrix', () => {
  it('enumerates all 25 public chat methods', () => {
    expect(chatMatrices).toHaveLength(25)
    expect(new Set(chatMatrices.map((matrix) => matrix.method)).size).toBe(25)
  })
